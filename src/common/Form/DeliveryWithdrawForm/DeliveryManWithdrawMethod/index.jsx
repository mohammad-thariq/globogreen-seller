import { Button } from "@/common/Button";
import style from "../../DeliveryManForm/index.module.css";
import * as Yup from "yup";
import { Formik } from "formik";
import { statusConstantOption } from "@/constant/statusConst";
import { InputSelect } from "../../common/inputSelect";

export const DeliveryManWithdrawMethod = ({
  onSave,
  onClose,
  currentWithdrawMethodId,
  data,
  onUpdate,
  loading,
  button,
}) => {
  const schema = Yup.object({
    name: Yup.string().required("Name is Required"),
    minimum_amount: Yup.string().required("Min Amount is Required"),
    maximum_amount: Yup.string().required("Max Amountis Required"),
    withdraw_charge: Yup.string().required("Withdraw Charge is Required"),
    status: Yup.string().required("Status is Required"),
  });
  console.log(data, "data this");

  return (
    <div className={style.wrapper}>
      <Formik
        initialValues={{
          name: data?.name?.toLowerCase() || "",
          minimum_amount: data?.min_amount || "",
          maximum_amount: data?.max_amount || "",
          withdraw_charge: data?.withdraw_charge || "",
          status: data?.status + 1 || "",
          man_type: data?.man_type || "",
          idn_type: data?.idn_type || "",
          idn_num: data?.idn_num || "",
          idn_image: data?.idn_image || "",
          phone: data?.phone || "",
          password: "",
          c_password: "",
        }}
        validationSchema={schema}
        onSubmit={(values, actions) => {
          onUpdate
            ? onUpdate({
                id: currentWithdrawMethodId,
                name: values?.name,
                description: "hello",
                minimum_amount: values?.minimum_amount,
                maximum_amount: values?.maximum_amount,
                withdraw_charge: values?.withdraw_charge,
                status: values?.status - 1,
              })
            : onSave({
                name: values?.name,
                description: "hello",
                minimum_amount: values?.minimum_amount,
                maximum_amount: values?.maximum_amount,
                withdraw_charge: values?.withdraw_charge,
                status: values?.status - 1,
              });
          actions.setSubmitting(true);
        }}
      >
        {({
          values,
          errors,
          touched,
          handleChange,
          handleBlur,
          handleSubmit,
        }) => (
          <form role="form" className="w-350">
            <label>Name</label>
            <div className="mb-3">
              <input
                type="name"
                name="name"
                className="form-control"
                placeholder="Name"
                onChange={handleChange}
                onBlur={handleBlur}
                value={values.name}
              />
              <p
                style={{ marginTop: "5px", marginBottom: "5px", color: "red" }}
              >
                {errors.name && touched.name && errors.name}
              </p>
            </div>
            <label>Minimum Amount</label>
            <div className="mb-3">
              <input
                type="number"
                name="minimum_amount"
                className="form-control"
                placeholder="Minimum Amount"
                onChange={handleChange}
                onBlur={handleBlur}
                value={values.minimum_amount}
              />
              <p
                style={{ marginTop: "5px", marginBottom: "5px", color: "red" }}
              >
                {errors.minimum_amount &&
                  touched.minimum_amount &&
                  errors.minimum_amount}
              </p>
            </div>
            <label>Maximum Amount</label>
            <div className="mb-3">
              <input
                type="number"
                name="maximum_amount"
                className="form-control"
                placeholder="Maximum Amount"
                onChange={handleChange}
                onBlur={handleBlur}
                value={values.maximum_amount}
              />
              <p
                style={{ marginTop: "5px", marginBottom: "5px", color: "red" }}
              >
                {errors.maximum_amount &&
                  touched.maximum_amount &&
                  errors.maximum_amount}
              </p>
            </div>

            <label>Charge</label>
            <div className="mb-3">
              <input
                type="name"
                name="withdraw_charge"
                className="form-control"
                placeholder="Charge"
                onChange={handleChange}
                onBlur={handleBlur}
                value={values.withdraw_charge}
              />
              <p
                style={{ marginTop: "5px", marginBottom: "5px", color: "red" }}
              >
                {errors.withdraw_charge &&
                  touched.withdraw_charge &&
                  errors.withdraw_charge}
              </p>
            </div>

            <InputSelect
              label={"Status"}
              onBlur={handleBlur}
              onChange={handleChange}
              name={"status"}
              values={values?.status}
              isValue
              onData={statusConstantOption}
            />

            <p style={{ marginTop: "5px", marginBottom: "5px", color: "red" }}>
              {errors.status && touched.status && errors.status}
            </p>
            <div className={style.btnWrapper}>
              <Button
                name="Close"
                border="1px solid #23D24F"
                color="#000"
                onClick={() => onClose()}
              />
              <Button
                name={button}
                bg="#23d24f"
                type="submit"
                color="#fff"
                onClick={handleSubmit}
                isSubmitting={loading}
              />
            </div>
          </form>
        )}
      </Formik>
    </div>
  );
};
